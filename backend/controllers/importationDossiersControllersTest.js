...
const updateDossiers = asyncHandler(async (req, res) => {
	try {
		...
		if (!importation_File) {
			if (!res.headersSent) res.status(400).send("No file uploaded");
		}
		...
		if (excelData.length > 0) {
			if (remark === "French Fichier Imported") {
				...
				if (!res.headersSent) res.status(400).send("Invalid remark provided");
			}
		} else {
			if (!res.headersSent) res.status(200).send("لا يمكن قراءة الملف");
		}
	} catch (error) {
...
async function processDossiers(data, creator, res, language) {
	try {
		...
		if (language === "numDos")
			if (!res.headersSent) res.status(200).send(numDos);
			else
				res
					.status(200)
					.send(
		...
		if (!res.headersSent) res.status(500).send("Error processing dossiers");
	}
}
...
function extractDossierData(dossier, language, res) {
	try {
		...
		if (!res.headersSent) res.status(500).send("Error extractDossierData");
	}
}
...
async function createConjoin(dossier1, language, creator, res) {
	try {
		...
		if (!res.headersSent) res.status(500).send("Error createConjoin");
	}
}
