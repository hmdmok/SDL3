  ...
  stream.on("end", function () {
    ...
    const dossiersMaped = excel_file?.map(
      asyncHandler(async (dossier) => {
        ...
        const {
          Nom: nom_dem_fr,
          Prenom: prenom_dem_fr,
          "N°\nDE ACT": num_act_dem,
          "Date de naissance": date_n_dem,
          "Lieu de naissance": lieu_n_dem_fr,
          sexe: gender,
          "Prénom du pére": prenom_p_dem_fr,
          "Nom de la mére": nom_m_dem_fr,
          "Prenom de la mére": prenom_m_dem_fr,
          "S F ": stuation_f,
          "Ref demande": num_dos,
          "Date demande": date_depo,
            "Nom DE CONJOINT": nom_conj_fr,
          "Prenom DE CONJOINT": prenom_conj_fr,
          "Date de naissance_1": date_n_conj,
          "N DE L ACT": num_act_conj,
          "Lieu de naissance_1": lieu_n_conj_fr,
          "Prénom du pére_1": prenom_p_conj_fr,
          "Nom de la mére_1": nom_m_conj_fr,
          "Prénom de la mére": prenom_m_conj_fr,
          Remarque: remark,
        } = dossier;
    ...
    Promise.all(dossiersMaped).then(() => {
      console.log(sheet_name[0]);
      var stream2 = reader.stream.to_json(file.Sheets[sheet_name[0]], {
        raw: false,
      });

      var excel_file2 = [];
      stream2.on("data", function (data) {
        excel_file2.push(data);
      });
      console.log(stream2);
      stream2.on("end", function () {
        const dossiersCount = excel_file2.length;
        var dossierAddedCount = 0;
        var dossierUpdatededCount = 0;

        const dossiersMaped = excel_file2?.map(
          asyncHandler(async (dossier) => {
            // // extract demandeur
            console.log(dossier);
            const {
            ...
            if (dossierToUpdate.length > 0) {
              if (dossierToUpdate[0].id_demandeur) {
                ...
                if (demandeurToUpdate?._id) {
                  ...
                  demandeurToUpdate.prenom_m =
                    prenom_m_dem || demandeurToUpdate.prenom_m;
                  demandeurToUpdate.nom_m =
                    nom_m_dem || demandeurToUpdate.nom_m;
                  const updatedDemandeur = await demandeurToUpdate.save();
                }
              }
              if (dossierToUpdate[0].id_conjoin) {
                ...
                // update conjoin
                if (conjoinToUpdate?._id) {
                  conjoinToUpdate.prenom =
                    prenom_conj || conjoinToUpdate.prenom;
                  conjoinToUpdate.nom = nom_conj || conjoinToUpdate.nom;
                  conjoinToUpdate.lieu_n =
                    lieu_n_conj || conjoinToUpdate.lieu_n;
                  conjoinToUpdate.prenom_p =
                    prenom_p_conj || conjoinToUpdate.prenom_p;
                  conjoinToUpdate.prenom_m =
                    prenom_m_conj || conjoinToUpdate.prenom_m;
              ...
              dossierToUpdate[0].notes = notes || dossierToUpdate[0].notes;
              dossierToUpdate[0].remark = remark || dossierToUpdate[0].remark;
              dossierToUpdate[0].num_dos =
                num_dos || dossierToUpdate[0].num_dos;
              const updatedDossier = await dossierToUpdate[0].save();
              dossierUpdatededCount++;
            } else {
              // check donnee valide
